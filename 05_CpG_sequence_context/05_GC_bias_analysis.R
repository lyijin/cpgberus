...
p = ggplot(Merged_dataframe_filt_long, aes(x = GC, y = value, color = Library_type, group = Library_type)) + geom_line(aes(color = Library_type, group = Sample_name), alpha = 0.5) + 
    stat_summary(geom = "line", fun = mean, size = 1.5) + scale_y_continuous(breaks = breaks_pretty(10), labels = comma) + scale_x_continuous(breaks = breaks_pretty(10), labels = comma) + 
    labs(x = "GC %", y = "Value") + facet_wrap( ~ variable,  scales="free_y") + theme_bw()

png(paste0(full_path, "GC_bias.png"), units="in", width=11.7, height=8.3, res=300)
print(p)
dev.off()
