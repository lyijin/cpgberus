...
covs_epic = lapply(samples, function(s) {
  ...
  queryHits(hits)
  mcols(x)$EPIC = mcols(epic_gr[subjectHits(hits)])[, epic_s]
  mcols(x)$IlmnID = names(epic_gr[subjectHits(hits)])
  mcols(x)$Type = manifest[mcols(x)$IlmnID, "Infinium_Design_Type"]
  x
})
names(covs_epic) = samples

# Form a GRangesList object
covs_epic = do.call("GRangesList", covs_epic)

save(covs_epic, file=file.path(path_cpgberus_covs, "grch38p13_combined_covs_epic-overlap_grl.RData"))
