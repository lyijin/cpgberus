...
"> three-way_per-pos_cov.R <

Reads in coverages from EM-seq, WGBS and ONT Cas9 experiments, and compares them 
on a per-position basis.

Shares the same cutoffs used in the beta script, so that outputs from both
...
# only keep relevant columns (keep coverages from this point on)
emseq_df <- emseq_df[, c('pos', 'cov', 'sample_id')]
wgbs_df <- wgbs_df[, c('pos', 'cov', 'sample_id')]
ont_df <- ont_df[, c('pos', 'cov', 'sample_id')]

# convert long-to-wide
emseq_wide_df <- reshape(emseq_df, idvar='pos', timevar='sample_id', direction='wide')
colnames(emseq_wide_df) <- gsub('^cov.', '', colnames(emseq_wide_df))
...
colnames(wgbs_wide_df) <- gsub('^cov.', '', colnames(wgbs_wide_df))
...
# hmm, not a lot. drop positions where coverage values were NA in ANY of the
# four datasets
...
# calculate per-position MEAN coverage values for every method separately
...
print(g)
ggsave('three-way.cov.pca.pdf', width=6, height=6)
...
ggplot(wide_df, aes(x=meanER, y=meanWR)) +
  ...
  ggtitle('Per-position coverage values, EM-seq vs. WGBS') +
  xlab('EM-seq') +
  ylab('WGBS') +
  theme_minimal(12)
...
ggplot(wide_df, aes(x=meanER, y=meanO)) +
  ...
  ggtitle('Per-position coverage values, EM-seq vs. ONT Cas9') +
  xlab('EM-seq') +
  ylab('ONT Cas9') +
  theme_minimal(12)
...
ggplot(wide_df, aes(x=meanWR, y=meanO)) +
  ...
  ggtitle('Per-position coverage values, WGBS vs. ONT Cas9') +
  xlab('WGBS') +
  ylab('ONT Cas9') +
  theme_minimal(12)
...
g1 <- ggplot(long_df, aes(x=pos, y=beta, color=method)) +
  ...
  ggtitle('Coverage values and GC% across 45S loci') +
  xlab('Position') +
  ylab('Coverage value') +
  theme_minimal(12) +
...
ggsave('three-way.cov_across_loci.pdf', width=10, height=5)

# does GC% influence the discrepancies in cov? re-do scatterplot, but colour
# points by GC%
# 
...
g3 <- ggplot(wide_df, aes(x=meanER, y=meanWR, color=gcpct)) +
  ...
  annotate('text', x=-Inf, y=Inf, hjust=0, vjust=1,
           label=lm_eqn(wide_df, 'meanER', 'meanWR'), parse=TRUE) +
  ggtitle('Per-position coverage values') +
  xlab('EM-seq') +
  ylab('WGBS') +
  theme_minimal(12) +
...
g6 <- ggplot(wide_df, aes(x=gcpct, y=delta_wgbs_emseq)) +
  ...
  ggtitle('Per-position residual coverage values vs. GC%') +
  xlab('GC%') +
...
ggsave('three-way.cov_and_gc.pdf', width=10, height=12)
